...
class Task(BaseModel):
    ...
    depends_on: list[str] = Field(
        default=[], description="CLI ordering: list of task IDs this depends on"
    )
    ...
    def applies_to_release_type(self, release_type: ReleaseType) -> bool:
        ...
        return (
            ReleaseType.ALL in self.release_types or release_type in self.release_types
        )
...
class ProjectMetadata(BaseModel):
    ...
    field_ids: dict[str, str] = Field(
        default_factory=dict, description="Mapping of field names to GitHub field IDs"
    )
...
class TaskList(BaseModel):
    ...
    def filter_by_release_type(self, release_type: ReleaseType) -> list[Task]:
        ...
        return [
            task for task in self.tasks if task.applies_to_release_type(release_type)
        ]
