...
from models import ProjectMetadata, Category, Priority, Phase, Task
...
def get_project_fields(project_number: int, owner: str = "@me") -> Dict[str, Any]:
    ...
    """
    cmd = ["gh", "project", "field-list", str(project_number), "--owner", owner, "--format", "json"]
    ...
    try:
        ...
        for field in fields_data:
            ...
            if "options" in field:
                field_info["options"] = {opt["name"]: opt["id"] for opt in field["options"]}
...
def create_item(project_number: int, owner: str, title: str, body: str = None) -> str:
    """
    Creates a draft issue item. Returns the Item Node ID.
    """
    cmd = ["gh", "project", "item-create", str(project_number), "--owner", owner, "--title", title, "--format", "json"]
    if body:
...
def update_item_field(project_id: str, item_id: str, field_id: str, value: str, is_single_select: bool = False):
    """
...
def add_tasks_to_project(project: ProjectMetadata, tasks: list[Task], dry_run: bool = False):
    """
    Iterates through tasks and creates them in the project with custom fields.
    """
    if dry_run:
        print(f"[Dry Run] Would create {len(tasks)} tasks in project {project.project_number}")
        return

    # Fetch real field IDs mapping
    ...
    for task in tasks:
        ...
        try:
            # 1. Create Item
            item_id = create_item(project.project_number, project.owner, task.title, task.body)
            ...
            if "Category" in fields_map:
                ...
                if opt_id:
                    update_item_field(project.id, item_id, field["id"], opt_id, is_single_select=True)
